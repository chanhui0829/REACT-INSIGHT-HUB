...
export function TopicList({
  ...
  const updateNicknames = async (topics: Topic[]) => {
    const authorIds = [...new Set(topics.map((t) => t.author))];
    const newNicknames = await getUserNicknames(authorIds);
    setNicknames((prev) => ({ ...prev, ...newNicknames }));
  };

  // 카테고리나 정렬이 바뀌면 페이지와 목록 초기화
  useEffect(() => {
    setPage(1);
  ...
  useEffect(() => {
    updateNicknames(initialTopics);
  }, [initialTopics]);

  // 페이지 바뀔 때마다 데이터 누적 및 닉네임 조회
  useEffect(() => {
    if (data?.topics) {
      setAllTopics((prev) => [...prev, ...data.topics]);
      updateNicknames(data.topics); // 새로 불러온 데이터의 닉네임 조회
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {allTopics.map((topic, index) => (
          <TopicCard
            key={`${topic.id}-${index}`}
            topic={topic}
            authorNickname={nicknames[topic.author]}
          />
        ))}
      </div>
